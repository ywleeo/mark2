...
- AI 网关 API 规范：[docs/server-api.md](docs/server-api.md)（mark2 ↔ mark2app.com server 协议）

## 本地 server 联调

`server/` 目录是 [docs/server-api.md](docs/server-api.md) 的本地 mock 实现（Node + Express + SQLite），用于客户端激活流程和 AI 调用联调。启动方式见 [server/README.md](server/README.md)。客户端联调时把 baseURL 指向 `http://localhost:8787` 即可。
