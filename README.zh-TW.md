...
它把檔案管理、Markdown 編輯、原始碼模式、文件大綱、程式碼查看、公式與 Mermaid 圖表、PDF 閱讀和卡片匯出放在同一個工作區。AI 不作為常駐側邊欄打斷寫作，而是貼近具體上下文：在游標附近續寫，在選取內容上潤色、擴寫、精簡，也可以對目前文件執行一次明確的處理指令。

![Mark2 主介面](demo/主界面.png)
...
- **文件能力完整**：除了 Markdown，Mark2 也支援 PDF、程式碼、圖片、影音、表格、Word 匯入和 Mermaid 圖表。它不是只會編輯 `.md` 文件，而是圍繞真實文件使用場景，把閱讀、寫作、整理和輸出放在一起。
...
游標所在行會出現輕量的 AI 入口。你可以讓 AI 基於全文上下文繼續往下寫，生成內容先以 ghost text 出現，用 `Tab` 接受、`Esc` 取消，確認後再寫入文件。

![AI 續寫](demo/AI%20续写.png)

#### 處理目前文件

AI 文件助手側邊欄會把每次輸入都視為新的目前任務。模型可以自行讀取目前文件、上一次工作稿或初始任務，也可以規劃多個子任務後綜合結果；程式不透過關鍵詞或介面狀態預判任務類型。結果會保留在側邊欄中，可繼續編輯、複製、插入或打開為臨時文件。

![AI 文件助手](demo/AI%20文档助手.png)

### 寫作與閱讀

Mark2 支援所見即所得編輯和原始碼模式，適合從草稿、筆記、技術文件到長文章的不同寫作習慣。編輯區支援自適應頁面寬度，也可以手動調整閱讀邊距；右側文件大綱幫助你快速定位和跳轉，適合長文件的閱讀與梳理。
...
程式碼檔案可以直接在工作區裡打開和編輯，支援語法高亮、行號和中文註解；Markdown 裡的程式碼區塊也做了更適合閱讀的展示和複製互動。

![程式碼編輯](demo/代码编辑.png)

#### Mermaid 圖表

在 Markdown 中直接渲染 Mermaid 流程圖、時序圖等圖表，適合把架構、流程和關係畫在文件裡。

![Mermaid 圖表](demo/mermaid.png)

### 卡片匯出

選取文件內容後可以匯出成圖片卡片，內建多種排版風格，適合把筆記、摘錄、觀點或文章片段發布到社群平台。
