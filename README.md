...
它把文件管理、Markdown 编辑、源码模式、文档大纲、代码查看、公式与 Mermaid 图表、PDF 阅读和卡片导出放在同一个工作区。AI 不作为常驻侧边栏打断写作，而是贴近具体上下文：在光标附近续写，在选中内容上润色、扩写、精简，也可以基于当前文档执行一次明确的处理指令。

![Mark2 主界面](demo/主界面.png)
...
- **文档能力完整**：除了 Markdown，Mark2 也支持 PDF、代码、图片、音视频、表格、Word 导入和 Mermaid 图表。它不是只会编辑 `.md` 文件，而是围绕真实文档使用场景，把阅读、写作、整理和输出放在一起。
...
光标所在行会出现轻量的 AI 入口。你可以让 AI 基于全文上下文继续往下写，生成内容先以 ghost text 出现，用 `Tab` 接受、`Esc` 取消，确认后再写入文档。

![AI 续写](demo/AI%20续写.png)

#### 处理当前文档

AI 文档助手侧边栏会把每次输入都视为新的当前任务。模型可以自行读取当前文档、上一次工作稿或初始任务，也可以规划多个子任务后综合结果；程序不通过关键词或界面状态预判任务类型。结果会保留在侧边栏中，可继续编辑、复制、插入或打开为临时文档。

![AI 文档助手](demo/AI%20文档助手.png)

### 写作与阅读

Mark2 支持所见即所得编辑和源码模式，适合从草稿、笔记、技术文档到长文章的不同写作习惯。编辑区支持自适应页面宽度，也可以手动调整阅读边距；右侧文档大纲帮助你快速定位和跳转，适合长文档的阅读与梳理。
...
代码文件可以直接在工作区里打开和编辑，支持语法高亮、行号和中文注释；Markdown 里的代码块也做了更适合阅读的展示和复制交互。

![代码编辑](demo/代码编辑.png)

#### Mermaid 图表

在 Markdown 中直接渲染 Mermaid 流程图、时序图等图表，适合把架构、流程和关系画在文档里。

![Mermaid 图表](demo/mermaid.png)

### 卡片导出

选中文档内容后可以导出成图片卡片，内置多种排版风格，适合把笔记、摘录、观点或文章片段发布到社交平台。

![生成卡片](demo/生成%20card.png)
...
Mark2 支持简体中文、繁体中文和英文。在「设置 > 通用 > 语言」中切换即可生效。
