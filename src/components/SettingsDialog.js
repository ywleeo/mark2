...
export class SettingsDialog {
  constructor(options = {}) {
    this.onSubmit =
      typeof options.onSubmit === "function" ? options.onSubmit : null;
    this.onCancel =
    ...
    this.appearanceSelect = this.form.querySelector(
      'select[name="appearance"]',
    );
    this.fontFamilySelect = this.form.querySelector(
    ...
    this.fontWeightSelect = this.form.querySelector(
    ...
    this.codeFontFamilySelect = this.form.querySelector(
      'select[name="codeFontFamily"]',
    );
    this.codeFontSizeInput = this.form.querySelector(
      'input[name="codeFontSize"]',
    );
    this.codeLineHeightInput = this.form.querySelector(
      'input[name="codeLineHeight"]',
    );
    this.codeFontWeightSelect = this.form.querySelector(
      'select[name="codeFontWeight"]',
    );
    // AI 助手设置字段
    this.aiCreativitySelect = this.form.querySelector(
      'select[name="aiCreativity"]',
    );
    this.aiKeysListEl = this.root.querySelector('[data-ref="aiKeysList"]');
    this.assistantModelSelectEl = this.root.querySelector(
      '[data-ref="assistantModelSelect"]',
    );
    this.fastModelSelectEl = this.root.querySelector(
      '[data-ref="fastModelSelect"]',
    );
    this.aiConfiguredProviders = []; // [{ id, apiKey, isCustom?, name?, baseUrl?, models? }]

    // 快捷键设置
    this.keybindingsContainerEl = this.root.querySelector(
      '[data-ref="keybindingsContainer"]',
    );
    this.keybindingsSettings = null;

    // 默认应用按钮（仅 macOS 渲染）
    this.defaultAppBtn = this.root.querySelector('[data-ref="defaultAppBtn"]');
    this.defaultAppExtsEl = this.root.querySelector(
      '[data-ref="defaultAppExts"]',
    );
    if (this.defaultAppBtn) {
      const cleanup = addClickHandler(this.defaultAppBtn, () =>
        this._onDefaultAppClick(),
      );
  ...
  async open(settings) {
    ...
    this._setSelectValue(
      this.codeFontWeightSelect,
      String(editorPrefs.codeFontWeight || 400),
    );
    // AI 助手设置 - 从 aiService 读取
    const aiConfig = await this.loadAiConfig();
    this.aiConfiguredProviders = (aiConfig.providers || []).map((p) => ({
      ...p,
    }));
    this._setSelectValue(
      this.aiCreativitySelect,
      aiConfig.preferences?.creativity || "medium",
    );
  ...
  async _refreshDefaultAppStatus() {
    ...
    try {
      const status = await invoke("get_default_app_status", {
        extensions: DEFAULT_APP_EXTENSIONS,
      });
      ...
      this._renderDefaultAppExts(
        DEFAULT_APP_EXTENSIONS.map((ext) => ({
          extension: ext,
          isSelf: false,
        })),
      );
  ...
  async _onDefaultAppClick() {
    ...
    this.defaultAppBtn.disabled = true;
    try {
      const results = await invoke("set_as_default_app", {
        extensions: DEFAULT_APP_EXTENSIONS,
      });
      ...
      if (failed.length > 0) {
        console.warn(
          "[Settings] some extensions failed to set default:",
          failed,
        );
      }
    } catch (error) {
  ...
  async handleSubmit(event) {
    ...
    const normalizedSize = Number.isFinite(fontSize)
      ? this.clamp(fontSize, 10, 48)
      : 16;
    const normalizedLineHeight = Number.isFinite(lineHeight)
      ? this.clamp(lineHeight, 1.0, 3.0)
      : 1.6;
    ...
    const normalizedCodeSize = Number.isFinite(codeFontSize)
      ? this.clamp(codeFontSize, 10, 48)
      : 14;
    const normalizedCodeLineHeight = Number.isFinite(codeLineHeight)
      ? this.clamp(codeLineHeight, 1.0, 3.0)
      : 1.5;

    const sanitized = {
      theme: theme,
      appearance: ["light", "dark", "system"].includes(appearance)
        ? appearance
        : "system",
      ...
      terminalFontFamily: (
        this.initialSettings?.terminalFontFamily || ""
      ).trim(),
    };
  ...
  syncFontWeight(weight) {
    ...
    this._setSelectValue(
      this.fontWeightSelect,
      allowed.includes(value) ? value : "400",
    );
  }
  ...
  _renderAiKeysList() {
    ...
    const cleanup = addClickHandler(addBtn, () =>
      this._showAddProviderForm(addBtn),
    );
    this.cleanupFunctions.push(cleanup);
  ...
  _buildProviderRow(provider, index) {
    ...
    if (provider.isCustom) {
      ...
      nameInput.addEventListener("input", () => {
        this.aiConfiguredProviders[index].name = nameInput.value;
      });
      ...
      urlInput.addEventListener("input", () => {
        this.aiConfiguredProviders[index].baseUrl = urlInput.value.trim();
      });
      ...
      modelsInput.addEventListener("input", () => {
        this.aiConfiguredProviders[index].models = modelsInput.value
          .split(",")
          .map((m) => m.trim())
          .filter(Boolean);
        this._refreshModelSelects();
    ...
    const cf = addClickHandler(fetchBtn, () =>
      this._fetchProviderModels(index),
    );
  ...
  _showAddProviderForm(addBtn) {
    ...
    const c1 = addClickHandler(cancelBtn, () => {
    ...
    providerSelect.addEventListener("change", () =>
      renderFields(providerSelect.value),
    );
    ...
    const c2 = addClickHandler(confirmBtn, () => {
      ...
      const getField = (name) =>
        fieldsEl.querySelector(`[data-field="${name}"]`)?.value?.trim() || "";
      ...
      if (selectedId === "__custom__") {
        ...
        this._refreshModelSelects();
        if (apiKey)
          this._fetchProviderModels(this.aiConfiguredProviders.length - 1);
        return;
      }
  ...
  _renderModelSelects(assistantModel, fastModel) {
    ...
    const assistantVal = assistantModel
      ? `${assistantModel.providerId}::${assistantModel.model}`
      : "";
    const fastVal = fastModel
      ? `${fastModel.providerId}::${fastModel.model}`
      : "";
    ...
    for (const el of [this.assistantModelSelectEl, this.fastModelSelectEl]) {
      if (this._dropdownMap.has(el)) {
        try {
          this._dropdownMap.get(el).destroy();
        } catch (_) {}
  ...
  async _fetchProviderModels(index) {
    ...
    const btn = this.aiKeysListEl?.querySelector(
