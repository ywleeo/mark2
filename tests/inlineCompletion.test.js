...
import { parseCompletionResponse } from '../src/features/inlineCompletion/CompletionResponseParser.js';
import {
    sanitizeCompletion,
...
test('返回解析器兼容分段正文并识别 reasoning-only 返回', () => {
    const segmented = parseCompletionResponse(JSON.stringify({
        choices: [{
            finish_reason: 'stop',
    ...
    const reasoningOnly = parseCompletionResponse(JSON.stringify({
        choices: [{
